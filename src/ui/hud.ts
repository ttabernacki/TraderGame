      ...
      append(this.course,
        ...
        el('div', { class: 'hud-big' },
          ...
          g.helmOrder !== null
            ...
            : coast
              ? `${coast.offingNm.toFixed(0)} miles off`
              : lat
        ...
        steer !== null
          ...
          : null,
        // The number the order is actually being held on, so the player can see
        // the master doing his job rather than having to trust him.
        coast
          ? hudRow('Land bears', `${g.sounding.shoreBearing.toFixed(0).padStart(3, '0')}° — `
            + `${g.sounding.shoreDistNm > 90 ? 'no bottom' : `${g.sounding.shoreDistNm.toFixed(1)} miles`}`)
          : null,
        ...
        dest && !lat && !coast ? hudRow('Distance', dest.distNm < 1
          ? 'less than a mile'
          : `${dest.distNm.toFixed(0)} miles`) : null,
        dest && !lat && !coast ? hudRow('At this rate', formatEta(dest.hours)) : null,
        g.route.length > 1
        ...
        dest && !lat && !coast && g.markDistNm > 1 ? progressBar(1 - dest.distNm / g.markDistNm) : null,
