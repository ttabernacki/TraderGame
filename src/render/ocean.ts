...
export interface WakeParams {
  ...
  rigHeight: number;
  /** Waterline to rail, which is where the solid part of that shadow ends. */
  hullHeight: number;
...
void main() {
  ...
  if (uSunDir.y > 0.08 && uShadow > 0.01 && dot(vLocal, vLocal) < 40000.0) {
    // A point at height h throws its shadow to (its position) minus
    // sunDir.xz/sunDir.y times h, so a patch of water at p is shadowed when
    // p + sunOff*h falls inside the ship at that height. Sweeping h from the
    // waterline to the masthead draws the whole silhouette — short and fat
    // with the sun overhead, long and thrown out to leeward of the sun when
    // it is low, which is what a ship's shadow on water does.
    //
    // It used to be two fixed blobs, one at the height of the hull and one at
    // the rig, with no sweep between them: the shadow hardly moved as the sun
    // crossed the sky and never took the shape of anything. Seven samples is
    // enough to leave no gap between the two.
    vec2 sunOff = uSunDir.xz / max(uSunDir.y, 0.16);
    vec2 fwd = uWakeDir;
    vec2 side = vec2(uWakeDir.y, -uWakeDir.x);
    float dark = 0.0;

    // The hull: a long low box, from the waterline to the rail.
    for (int i = 0; i < 3; i++) {
      float h = uHullHeight * (float(i) + 0.5) / 3.0;
      vec2 q = vLocal + sunOff * h;
      vec2 uv = vec2(dot(q, fwd) / uShipHalfLength, dot(q, side) / uShipHalfBeam);
      dark = max(dark, 1.0 - smoothstep(0.72, 1.12, length(uv)));
    }
    // The rig and what is hanging in it: narrower than the hull athwartships,
    // most of its area forward, and thinning toward the truck where there is
    // nothing but spars and rope.
    for (int i = 0; i < 4; i++) {
      float f = (float(i) + 0.5) / 4.0;
      float h = mix(uHullHeight, uRigHeight, f);
      vec2 q = vLocal + sunOff * h;
      vec2 uv = vec2(dot(q, fwd) / (uShipHalfLength * 0.85),
                     dot(q, side) / (uShipHalfBeam * 1.45));
      dark = max(dark, (1.0 - smoothstep(0.5, 1.15, length(uv))) * mix(0.95, 0.5, f));
    }

    // Shadowed water is not simply darker water: it has lost the sun and kept
    // the sky, so it goes toward the deep body colour and the glitter goes out
    // of it. A flat multiply left the sun's track burning through her shadow.
    float k = clamp(dark, 0.0, 1.0) * uShadow
            // A sun near the horizon throws a shadow so long and so raked that
            // it has no edge left worth drawing.
            * smoothstep(0.08, 0.30, uSunDir.y);
    col = mix(col, mix(col * 0.62, uDeepColor, 0.28), k);
  }
...
export class Ocean {
  ...
  setSea(p: OceanParams, dEast: number, dNorth: number, dt: number): void {
    ...
    // A flat calm is not flat.
    //
    // The weather calls a light breeze a tenth of a metre, and a tenth of a
    // metre drawn honestly is a sheet of glass with a ship glued to it — no
    // swell, no bobbing, nothing in the picture moving at all. There is always
    // old swell running in the open ocean, generated by weather a thousand
    // miles away and days ago, and it does not go away because the local wind
    // has dropped. So the *drawn* sea carries a floor of about a foot and a
    // half, under anything the weather would dignify with a sea state. The
    // physics is untouched: this is what the water looks like, not what it
    // does to her.
    const drawnHeight = Math.max(this.seaHeight, 0.45);
    ...
    const baseAmp = drawnHeight * 0.275;
    // Peak wavelength of a fully developed sea: 0.79 U squared in metres, which
    // in knots is a shade over a fifth of the square of the wind. Floored with
    // the height: residual swell is long, and a calm drawn with the wavelength
    // its own four knots would raise gives a ripple a few metres from crest to
    // crest, which reads as a pond rather than as an ocean.
    const drawnWind = Math.max(this.seaWind, 9);
    const baseLen = clamp(5 + drawnWind * drawnWind * 0.21, 8, 520);
  ...
  setWake(w: WakeParams): void {
    ...
    this.material.uniforms.uHullHeight.value = Math.max(w.hullHeight, 0.5);
